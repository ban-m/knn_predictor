...
fn getfilenames(path:&Path) -> std::io::Result<Vec<PathBuf>>{
    let mut files:Vec<_> = fs::read_dir(path)?.filter_map(|e| e.ok()).
...
fn getsam(path:&Path) -> std::io::Result<HashMap<String,usize>>{
    Ok(BufReader::new(File::open(path)?).lines().filter_map(|e|e.ok()).
       filter_map(|line| {
           if line.starts_with('@'){
               None
           }else{
...
fn main(){
    ...
    eprintln!("sam setup");
    let result :Vec<_> = ecolifiles.par_iter().
        filter_map(|filename| match fast5wrapper::get_read_id(filename.to_str().unwrap()){
